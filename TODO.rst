...
- If continuing a property after a fail, do so only if the property is not 
  failing for _every_ try. Also, if a certain number of tries fail, stop. By 
  default, continue if only a few fails occur.
